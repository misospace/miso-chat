...
class GatewayWsManager extends EventEmitter {
  ...
  /**
   * Store pending requests for recovery after reconnection
   * @private
   */
  _storePendingForRecovery() {
    this._recoveryPending = new Map(this.pendingRequests);
    this.pendingRequests.clear();
  }

  /**
   * Recover pending requests after reconnection
   * @private
   */
  ...
  connect(origin = 'http://localhost:3000') {
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('close', (code, reason) => {
        ...
        // Store pending requests for recovery if this is a reconnect (not initial connect)
        if (wasConnected) {
          this._storePendingForRecovery();
        } else {
      ...
      this.ws.on('message', (data) => {
        try {
          ...
          if (!this.connected && this._frameMatchesId(frame, this._connectId)) {
            ...
            // Recover pending requests after successful reconnection
            this._recoverPendingRequests();
  ...
  /**
   * Get count of pending requests waiting for recovery after reconnection
   * @public
   * @returns {number} Number of pending requests waiting for recovery
   */
  getPendingForRecoveryCount() {
    return this._recoveryPending ? this._recoveryPending.size : 0;
  }
}
